...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        if completion_status["has_sufficient_info"]:
            # У нас достаточно информации - используем RAG с локальными документами
            ...
            # Используем query функцию из обновленного query.py с локальными документами
...
async def simple_chat(request: SimpleRequest):
    ...
    try:
        init_components()
        response = query(request.message)
...
def _handle_followup_question(message: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    response_parts.append("\n")
    response_parts.append("\n📚 Полный курс по банкротству")
    response_parts.append("\n   → Пошаговые инструкции")
    response_parts.append("\n   → Образцы документов") 
    response_parts.append("\n   → Поддержка экспертов")
    response_parts.append("\n")
    response_parts.append("\n📖 Учебник по банкротству в Казахстане")
    response_parts.append("\n   → Все законы и процедуры")
    response_parts.append("\n   → Реальные кейсы")
    response_parts.append("\n   → Актуальная информация 2024")
