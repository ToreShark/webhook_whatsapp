...
- Если последний вопрос содержал слово "доход" и пользователь отвечает "нет"/"нету" -> ОБЯЗАТЕЛЬНО устанавливай has_income: false
- Если последний вопрос содержал слово "недвижимость" и пользователь отвечает "нет"/"нету"/"нет говорю" -> ОБЯЗАТЕЛЬНО устанавливай has_property: false
- Если последний вопрос содержал слово "машина"/"автомобиль" и пользователь отвечает "нет" -> ОБЯЗАТЕЛЬНО устанавливай has_car: false
- ВСЕГДА устанавливай соответствующее поле в false при отрицательном ответе на конкретный вопрос
- Короткий ответ "нет" ВСЕГДА интерпретируй в контексте последнего вопроса
...
С контекстом последнего вопроса:
Вопрос: "Есть ли у вас недвижимость - квартира, дом, дача?" + Ответ: "нет" -> {{"has_property": false}}
Вопрос: "У вас есть официальный доход сейчас?" + Ответ: "нет" -> {{"has_income": false}}
Вопрос: "У вас есть официальный доход сейчас?" + Ответ: "нету" -> {{"has_income": false}}
